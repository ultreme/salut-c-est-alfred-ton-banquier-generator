...
(function() {
  ...
  Program = (function() {
    ...
    function Program(options) {
      ...
      this.sayAlfred = __bind(this.sayAlfred, this);
      ...
      if ((_ref2 = (_base2 = this.options).usage) == null) {
        _base2.usage = ["  Usage: salut-c-est-alfred-ton-banquier-generator [options] method [arguments...]", "", "    Options:", "", "       -v, --verbose        verbose", "       -o, --out <file>     output file", "       -V, --voice <voice>  voice", "", "    Commands:", "", "       list-voices", "       list-words", "       say <word1> <word2> <...>", "       say-random <length>", "       alfred <length>", ""];
      }
    ...
    Program.prototype.setupHandlers = function() {
      ...
      flatiron.app.cmd(/list-words/, this.listWords);
      flatiron.app.cmd(/say (.+)/, this.say);
      flatiron.app.cmd(/alfred (.+)/, this.sayAlfred);
      return flatiron.app.cmd(/say-random (.+)/, this.sayRandom);
    };

    Program.prototype.sayAlfred = function(length, fn) {
      var _this = this;
      if (length == null) {
        length = 20;
      }
      ...
      this.getAlfred();
      this.alfred.sayAlfredPass(length, function(err, data) {
    ...
    Program.prototype.listWords = function(fn) {
      var _this = this;
      if (fn == null) {
        fn = null;
      }
      this.getAlfred();
      this.alfred.listWords(function(err, voiceWords) {
        var voice, words, _results;
        _results = [];
        for (voice in voiceWords) {
          words = voiceWords[voice];
          _results.push(flatiron.app.log.info("" + voice + ": ", words.join(',')));
        }
        return _results;
      });
      if (fn) {
    ...
    Program.prototype.sayRandom = function(length, fn) {
      var _this = this;
      if (fn == null) {
        fn = null;
      }
      this.getAlfred();
      this.alfred.sayRandom(length, function(err, data) {
        return flatiron.app.log.info(err, data);
      });
      if (fn) {
        return fn();
